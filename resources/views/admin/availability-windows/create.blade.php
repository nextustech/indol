...
<form method="POST" action="{{ route('admin.availability-windows.store') }}">
@csrf
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            <input type="number" class="form-control @error('slot_duration') is-invalid @enderror" 
                   id="slot_duration" name="slot_duration" value="{{ old('slot_duration', 30) }}" required min="1">
            @error('slot_duration')
                <span class="invalid-feedback">{{ $message }}</span>
            @enderror
        </div>
